...
docker run --rm -it --name satorineuron -p 24600:24600 -p 24601:24601 -v "%APPDATA%\Satori\run:/Satori/Neuron/run" -v "%APPDATA%\Satori\wallet:/Satori/Neuron/wallet" -v "%APPDATA%\Satori\config:/Satori/Neuron/config" -v "%APPDATA%\Satori\data:/Satori/Neuron/data" -v "%APPDATA%\Satori\models:/Satori/Neuron/models" --env ENV=prod satorinet/satorineuron:latest ./start.sh
echo.
if %ERRORLEVEL% EQU 0 (
