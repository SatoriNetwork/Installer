...
// Function to check if a batch file is valid (basic validation: non-empty, contains expected commands)
bool isValidBatchFile(const std::filesystem::path& batchFilePath) {
    std::ifstream batchFile(batchFilePath);
    if (!batchFile) return false; // File couldn't be opened

    std::string line;
    while (std::getline(batchFile, line)) {
        if (line.find("docker") != std::string::npos) { // Look for expected command
            return true;
        }
    }
    return false;
...
int main() {
    try {
        ...
        // Paths for batch files
        std::filesystem::path batchPath = satoriPath / L"run/satori.bat";
        std::filesystem::path startupPath = std::filesystem::path(appdata) / L"Microsoft/Windows/Start Menu/Programs/Startup/satori.bat";

        // If the batch file exists and is valid, run it
        if (std::filesystem::exists(batchPath) && isValidBatchFile(batchPath)) {
            std::cout << "Found valid batch file. Running Satori...\n";
            ShellExecuteW(NULL, L"open", batchPath.c_str(), NULL, NULL, SW_SHOWNORMAL);
        } else {
            std::cout << "No valid batch file found. Creating default startup script...\n";
